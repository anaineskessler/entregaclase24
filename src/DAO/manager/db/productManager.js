  ...
  getProducts = async (limit, page, query, filter, sort) => {
    ...
    try {
      if (query && filter) {
        if (query === "title") {
          !sort
            ? (products = await productModel.find({ title: filter }))
            : (products = await productModel
                .find({ title: filter })
                .sort({ price: ordenar }));
        } else if (query === "status") {
          !sort
            ? (products = await productModel.find({ status: filter }))
            : (products = await productModel
                .find({ status: filter })
                .sort({ price: ordenar }));
        }
        ...
        !sort
          ...
          : (products = await productModel.find().sort({ price: ordenar }));
      }
      !page
        ? (products = products.slice(0, 10))
        : (products = products.slice(10 * page, 10 * page + 10));
      !limit
        ? (products = products.slice(0, 10))
